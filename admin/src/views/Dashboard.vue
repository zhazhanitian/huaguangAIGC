...
function updateGrowthChart() {
  ...
  ch.setOption({
    ...
    series: [{
      type: 'line', data: userGrowth.value.map(d => d.count), smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { width: 2.5, color: '#4080FF' },
...
function updateModelChart() {
  ...
  ch.setOption({
    tooltip: {
      trigger: 'axis', axisPointer: { type: 'shadow' }, backgroundColor: cTipBg, borderColor: cTipBd, textStyle: { color: '#e2e8f0', fontSize: 12 },
  ...
  <div class="dashboard">
    ...
    <div class="kpi-grid">
      <div v-for="card in kpiCards" :key="card.label" class="kpi glass-panel">
        <div class="kpi-top">
          <div class="kpi-badge" :class="'tone-' + card.tone">
    ...
    <div class="aigc-section glass-panel">
      ...
      <div class="aigc-grid">
        <div v-for="m in aigcModules" :key="m.label" class="aigc-card">
          <div class="aigc-top">
            <div class="aigc-badge" :class="'tone-' + m.tone">
              <component :is="m.icon" />
            </div>
    ...
    <div class="chart-row">
      ...
      <div class="glass-panel chart-panel">
        <div class="cp-head">
          <div class="cp-title">
            <IconArrowRise class="cp-icon" /> 收入趋势
          </div>
    ...
    <div class="chart-row">
      <div class="glass-panel chart-panel">
        <div class="cp-head">
          <div class="cp-title">
            <IconCloud class="cp-icon" /> Token 消耗趋势
          </div>
      ...
      <div class="glass-panel chart-panel">
        <div class="cp-head">
          <div class="cp-title">
            <IconRobot class="cp-icon" /> 模型使用 Top 10
          </div>
    ...
    <div class="glass-panel shortcut-section">
      <div class="cp-head">
        <div class="cp-title">
          <IconApps class="cp-icon" /> 快捷入口
        </div>
      </div>
      <div class="entry-grid">
        <div v-for="(s, i) in shortcuts" :key="i" class="entry-card" role="button" tabindex="0"
          @click="handleEntryClick(s)">
          <div class="entry-icon" :class="'tone-' + s.tone">
            <component :is="s.icon" />
          </div>
...
.tone-pink {
  background: rgba(247, 89, 171, 0.14);
  color: #F9A8D4;
}
