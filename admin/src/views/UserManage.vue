  ...
  <div class="user-manage">
    ...
    <div class="glow-card toolbar-card">
      <div class="toolbar">
        <div class="toolbar-row toolbar-row-filters">
          ...
          <a-select v-model="collegeId" placeholder="部门" allow-clear class="filter-select college"
            @change="onFilterCollegeChange">
            <a-option v-for="c in filterCollegeList" :key="c.id" :value="c.id" :label="c.name" />
          </a-select>
          <a-select v-model="gradeId" placeholder="区域" allow-clear :disabled="!collegeId" class="filter-select grade"
            @change="onFilterGradeChange">
            <a-option v-for="g in filterGradeList" :key="g.id" :value="g.id" :label="g.name" />
          </a-select>
          <a-select v-model="majorId" placeholder="团队" allow-clear :disabled="!gradeId" class="filter-select major"
            @change="onFilterMajorChange">
            <a-option v-for="m in filterMajorList" :key="m.id" :value="m.id" :label="m.name" />
          </a-select>
          <a-select v-model="classId" placeholder="项目组" allow-clear :disabled="!majorId" class="filter-select class">
    ...
    <a-modal v-model:visible="editDialogVisible" title="编辑用户" width="480px" unmount-on-close class="edit-modal"
      ...
      <a-form ref="formRef" :model="editForm" :rules="editRules" layout="horizontal" :label-col-props="{ span: 6 }"
        ...
        <a-form-item label="部门" field="collegeId">
        ...
        <a-form-item label="区域" field="gradeId">
          <a-select v-model="editForm.gradeId" placeholder="先选部门" allow-clear :disabled="!editForm.collegeId"
            style="width: 100%" @change="onEditFormGradeChange">
            <a-option v-for="g in gradeOptionsForForm" :key="g.id" :value="g.id" :label="g.name" />
          </a-select>
        </a-form-item>
        <a-form-item label="团队" field="majorId">
          <a-select v-model="editForm.majorId" placeholder="先选区域" allow-clear :disabled="!editForm.gradeId"
            style="width: 100%" @change="onEditFormMajorChange">
            <a-option v-for="m in majorOptionsForForm" :key="m.id" :value="m.id" :label="m.name" />
          </a-select>
        </a-form-item>
        <a-form-item label="项目组" field="classId">
          <a-select v-model="editForm.classId" placeholder="先选团队" allow-clear :disabled="!editForm.majorId"
            style="width: 100%">
    ...
    <a-modal v-model:visible="addDialogVisible" title="添加用户" width="520px" unmount-on-close class="edit-modal"
      ...
      <a-form ref="addFormRef" :model="addForm" :rules="addRules" layout="horizontal" :label-col-props="{ span: 6 }"
        ...
        <a-form-item label="区域" field="gradeId">
          <a-select v-model="addForm.gradeId" placeholder="先选部门" allow-clear :disabled="!addForm.collegeId"
            style="width: 100%" @change="onAddFormGradeChange">
            <a-option v-for="g in gradeOptionsForForm" :key="g.id" :value="g.id" :label="g.name" />
          </a-select>
        </a-form-item>
        <a-form-item label="团队" field="majorId">
          <a-select v-model="addForm.majorId" placeholder="先选区域" allow-clear :disabled="!addForm.gradeId"
            style="width: 100%" @change="onAddFormMajorChange">
            <a-option v-for="m in majorOptionsForForm" :key="m.id" :value="m.id" :label="m.name" />
          </a-select>
        </a-form-item>
        <a-form-item label="项目组" field="classId">
          <a-select v-model="addForm.classId" placeholder="先选团队" allow-clear :disabled="!addForm.majorId"
            style="width: 100%">
            <a-option v-for="cl in classOptionsForForm" :key="cl.id" :value="cl.id" :label="cl.name" />
          </a-select>
        </a-form-item>
...
/* 纯 Grid：5 列，前 5 项第一行，后 3 项第二行；团队/项目组与首行同列宽，按钮组不拉伸 */
.toolbar-row-filters {
  display: grid;
