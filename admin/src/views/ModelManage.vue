...
const imageModelNames = new Set([
  'nano-banana-pro', 'nano-banana-2', 'nano-banana-fast', 'nano-banana', 'nano-banana-pro-vt',
...
// 默认只看 MAPI 模型，减少误操作（MAPI 聚合切换后非 MAPI 模型通常均已禁用）
const filterSource = ref<'all' | 'mapi' | 'preset' | 'local'>('mapi')
...
const filteredModels = computed(() => {
  return modelList.value.filter(m => {
    ...
    if (filterSource.value !== 'all' && (m.source || 'local') !== filterSource.value) return false
...
function providerColor(provider: string) {
  if (provider === 'mapi') return '#7C3AED'
...
async function fetchModels() {
  ...
  try {
    ...
    modelList.value = list.map((m: any) => ({
      ...
      isPublic: toBool(m?.isPublic ?? true),
...
async function handleSyncMapi() {
  Modal.confirm({
    title: '同步 MAPI 模型目录',
    content: '将从 MAPI 拉取模型目录并同步到本地模型库。新同步的 MAPI 模型默认启用并在前台展示；已存在的模型保持现有启停状态。',
    okText: '开始同步',
    cancelText: '取消',
    onOk: async () => {
      submitLoading.value = true
      try {
        const res = await syncMapiModels()
        Message.success(`MAPI 同步完成：新增 ${res.created}，更新 ${res.updated}`)
        await fetchModels()
      } finally {
        submitLoading.value = false
      }
...
async function handleDisableNonMapi() {
  Modal.confirm({
    title: '一键关闭非 MAPI 模型',
    content: '将把数据库中所有 source 不等于 mapi 的模型 isActive/isPublic 置为 false，用户端与模型广场不再展示它们。此操作可逆，随时可重新启用。是否继续？',
    okText: '立即关闭',
    cancelText: '取消',
    onOk: async () => {
      submitLoading.value = true
      try {
        const res = await bulkDisableModels({ sourceNot: 'mapi' })
        Message.success(`已关闭 ${res.affected} 个非 MAPI 模型`)
        await fetchModels()
      } finally {
        submitLoading.value = false
      }
...
function openAdd() {
  addForm.modelName = ''
  addForm.displayName = ''
  addForm.provider = 'mapi'
  addForm.type = 'text'
  addForm.apiKey = ''
  addForm.baseUrl = ''
  addForm.isActive = true
  addForm.isPublic = true
  addForm.description = ''
...
function openEdit(row: Model) {
  ...
  editForm.displayName = row.displayName || row.modelName
  ...
  editForm.isPublic = row.isPublic ?? true
  ...
  editForm.upstreamModelId = row.upstreamModelId || ''
  editForm.capabilityTags = row.capabilityTags || ''
  editForm.rawMetadata = row.rawMetadata || ''
...
async function submitEdit() {
  ...
  try {
    await updateModel(editForm.id, {
      ...
      upstreamModelId: editForm.upstreamModelId || undefined,
      capabilityTags: editForm.capabilityTags || undefined,
      rawMetadata: editForm.rawMetadata || undefined,
...
const togglingPublicId = ref<string | null>(null)
...
async function toggleModelPublic(model: Model, nextVal: boolean) {
  if (togglingPublicId.value === model.id) return
  const prev = !!model.isPublic
  model.isPublic = nextVal
  togglingPublicId.value = model.id
  try {
    await updateModel(model.id, { isPublic: nextVal })
    Message.success(nextVal ? '已在前台展示' : '已从前台隐藏')
  } catch {
    model.isPublic = prev
  } finally {
    togglingPublicId.value = null
  }
}
  ...
  <div class="model-manage">
    ...
    <div class="glow-card header-card">
      ...
      <div class="header-right">
        <a-button class="sync-btn" :loading="submitLoading" @click="handleSyncMapi">
          <template #icon>
            <IconRefresh />
          </template>
          同步 MAPI
        </a-button>
        <a-button class="sync-btn" status="warning" :loading="submitLoading" @click="handleDisableNonMapi">
          <template #icon>
            <IconDelete />
          </template>
          关闭非 MAPI 模型
    ...
    <div class="glow-card toolbar-card">
      <div class="toolbar">
        <div class="toolbar-right">
          ...
          <a-select v-model="filterSource" :style="{ width: '120px' }" placeholder="来源">
            <a-option value="all">全部来源</a-option>
            <a-option value="mapi">MAPI</a-option>
            <a-option value="preset">预设</a-option>
      ...
      <div v-show="viewMode === 'grid' && filteredModels.length" class="model-grid">
        <div v-for="model in filteredModels" :key="model.id" class="glow-card model-card"
          ...
          <div class="model-card-header" @click="toggleCardExpand(model.id)">
            ...
            <div class="model-card-info">
              ...
              <div class="model-meta">
                <span class="points-badge">{{ sourceText(model.source) }}</span>
          ...
          <div v-show="expandedCardId === model.id" class="model-card-expand">
            ...
            <div class="key-row">
              <span class="key-masked">前台展示</span>
              <a-switch :model-value="model.isPublic" size="small" checked-color="#00B42A"
                unchecked-color="rgba(255,255,255,0.12)" :loading="togglingPublicId === model.id"
                @change="(val) => toggleModelPublic(model, Boolean(val))" />
              <span class="key-extra">{{ model.isPublic ? '展示中' : '已隐藏' }}</span>
            </div>
            <div v-if="!(model.keys?.length)" class="empty-keys">暂无 API Key</div>
      ...
      <div v-show="viewMode === 'table' && filteredModels.length" class="glow-card table-card">
        <a-table :columns="columns" :data="filteredModels" :loading="loading" row-key="id" :expandable="{
          ...
          <template #source="{ record }">
            <span class="provider-pill">{{ sourceText(record.source) }}</span>
          </template>
          ...
          <template #public="{ record }">
            <div class="table-switch-cell">
              <a-switch :model-value="record.isPublic" :loading="togglingPublicId === record.id" checked-color="#00B42A"
                unchecked-color="rgba(255,255,255,0.12)" @change="(val) => toggleModelPublic(record, Boolean(val))">
                <template #checked>ON</template>
                <template #unchecked>OFF</template>
              </a-switch>
            </div>
    ...
    <a-modal v-model:visible="addDialogVisible" title="添加模型" width="480px" unmount-on-close class="model-modal">
      <a-form ref="formRef" :model="addForm" :rules="addRules" layout="horizontal" :label-col-props="{ span: 6 }"
        ...
        <a-form-item label="展示名称" field="displayName">
          <a-input v-model="addForm.displayName" placeholder="前台显示名称，可选" />
        </a-form-item>
        ...
        <a-form-item label="前台展示" field="isPublic">
          <a-switch v-model="addForm.isPublic" checked-color="#00B42A" unchecked-color="rgba(255,255,255,0.12)">
    ...
    <a-modal v-model:visible="editDialogVisible" title="编辑模型" width="480px" unmount-on-close class="model-modal">
      <a-form ref="formRef" :model="editForm" :rules="addRules" layout="horizontal" :label-col-props="{ span: 6 }"
        ...
        <a-form-item label="前台展示" field="isPublic">
          <a-switch v-model="editForm.isPublic" checked-color="#00B42A" unchecked-color="rgba(255,255,255,0.12)">
            <template #checked>展示</template>
            <template #unchecked>隐藏</template>
          </a-switch>
        </a-form-item>
        <a-form-item label="来源" field="source">
          <a-input v-model="editForm.source" placeholder="local / preset / mapi" />
        </a-form-item>
        <a-form-item label="上游ID" field="upstreamModelId">
          <a-input v-model="editForm.upstreamModelId" placeholder="可选，上游平台模型 ID" />
        </a-form-item>
      </a-form>
      <template #footer>
