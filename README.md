...
（端口以各目录下 `vite.config.ts` / `server` 的 `PORT` 为准；本地开发默认如下。）

- 用户端（创作端）: http://127.0.0.1:3002
- 管理后台: http://127.0.0.1:5174（`admin` 的 Vite 端口；勿与用户端混淆）
- API 文档: http://127.0.0.1:3001/api/docs（`server/.env` 里 `PORT` 默认 3001）

说明：管理员账号请在**管理后台**登录；若在创作端登录会被拦截并提示正确地址（见 `web/.env.development` 的 `VITE_ADMIN_URL`）。
