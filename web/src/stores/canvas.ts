...
/**
 * MAPI 聚合平台模型名启发式判断（避免 store 反向依赖 model api）。
 * 对齐 server/common/mapi-task-client.ts 中 isMapiModelName 的规则，
 * 但任何未命中的模型仍会被后端兜底识别（服务端会查 ai_models.source）。
 */
function isMapiCanvasModel(name?: string | null): boolean {
  ...
  async function createTaskForNode(id: string) {
    ...
    try {
      if (taskType === 'text2video' || taskType === 'img2video') {
        // MAPI 视频：按文档字段 ratio / duration / generate_audio / cameraFixed / watermark 传参
        // 画布节点暂未提供这些 UI，使用与 VideoLayout 一致的默认值；
        // 节点自有 params 中若已携带上述字段会覆盖默认。
        const baseParams: Record<string, unknown> = isMapiCanvasModel(provider)
          ? {
              ratio: '16:9',
              duration: 5,
              generate_audio: true,
          ...
          : { size: node.size, style: node.style, ...(node.params || {}) }
        ...
        // MAPI 图片：把参考图归一到 inputImage、并补上 watermark 默认字段，后端短路按 MAPI 文档处理
        if (isMapiCanvasModel(provider)) {
          const nodeParams = (node.params || {}) as Record<string, unknown>
          if (typeof nodeParams.watermark === 'boolean') {
            mergedParams.watermark = nodeParams.watermark
          } else if (typeof mergedParams.watermark !== 'boolean') {
            mergedParams.watermark = false
          }
        }
        ...
        if (isMapiCanvasModel(provider) && (taskType === 'img2img' || taskType === 'variation') && sourceUrl) {
          mergedParams.inputImage = mergedParams.inputImage || sourceUrl
        }
