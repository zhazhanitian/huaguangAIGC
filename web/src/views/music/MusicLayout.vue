...
const musicModelsFromApi = ref<Array<{
  modelName: string
...
const modelOptions = computed(() => {
  ...
  const knownBackendNames = new Set(Object.values(musicOptionToBackend))
  const dynamicList = musicModelsFromApi.value
  ...
  const list = [...baseList, ...dynamicList.filter(opt => set.size === 0 || set.has(opt.ptsKey))]
  return list.map(m => ({
    value: m.value,
    label: m.label,
