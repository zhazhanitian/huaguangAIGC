...
import Model3dParamForm from './Model3dParamForm.vue'
import { buildDefaultModel3dParams } from './model3d-schemas'
...
const form = ref({
  taskType: 'text2model' as 'text2model' | 'img2model',
  provider: 'tencent-hunyuan-3d-pro',
  prompt: '',
})
const providerParams = ref<Record<string, unknown>>(
  buildDefaultModel3dParams('tencent-hunyuan-3d-pro'),
)
...
const providersDef = [
  ...
  { value: 'tripo3d-text-to-model', label: 'Tripo 3D 文生模型', desc: 'Tripo 官方文生 3D，参数与官网保持一致' },
  { value: 'tripo3d-image-to-model', label: 'Tripo 3D 图生模型', desc: 'Tripo 官方图生 3D，适合单图转高质量 3D 资产' },
]
/** GET /model/list?type=3d 返回的 modelName 与前端 value 一致（来自 model.service 预设），无需映射 */
const model3dModelsFromApi = ref<Array<{
  modelName: string
...
const providers = computed(() => {
  ...
  const knownNames = new Set(providersDef.map(p => p.value))
  const dynamicProviders = model3dModelsFromApi.value
    .filter((m) => !knownNames.has(m.modelName))
    .map((m) => ({
      value: m.modelName,
      label: m.displayName || m.modelName,
      desc: m.description || `动态同步模型：${m.modelName}`,
    }))
  if (set.size === 0) return [...providersDef, ...dynamicProviders]
  return [
    ...providersDef.filter(p => set.has(p.value)),
...
async function fetch3dModels() {
  try {
    ...
    if (Array.isArray(list)) {
      model3dModelsFromApi.value = list
...
const isTripoProvider = computed(() => form.value.provider.startsWith('tripo3d'))
const promptMaxLength = computed(() => {
  if (isTripoProvider.value) return 2000
  return isProMode.value ? 1024 : 200
})

watch(
  () => form.value.provider,
  (provider) => {
    providerParams.value = buildDefaultModel3dParams(provider)
    if (provider === 'tripo3d-image-to-model') form.value.taskType = 'img2model'
    if (provider === 'tripo3d-text-to-model') form.value.taskType = 'text2model'
  },
  { immediate: true },
)

function updateProviderParams(value: Record<string, unknown>) {
  providerParams.value = value
}
...
async function handleGenerate() {
  const rawPrompt = form.value.prompt.trim()
  const allowEmptyPrompt =
    isTripoProvider.value && form.value.taskType === 'img2model'
  if (!rawPrompt && !allowEmptyPrompt) {
    Message.warning('请输入 3D 生成描述')
    return
  }
  const prompt = rawPrompt || 'Generate 3D model from reference image'
...
function handleExport(task?: Model3dTask | null) {
  const url = primaryDownloadUrl(task)
  if (!url) {
...
function taskParams(task?: Model3dTask | null): Record<string, unknown> {
  return ((task?.params || {}) as Record<string, unknown>) || {}
}

function taskDownloads(task?: Model3dTask | null): Array<{ key: string; label: string; url: string }> {
  ...
  const normalized = outputs
    .map((item) => {
      ...
      if (!url) return null
      return {
        key: String(record.key || 'file'),
        label: String(record.label || '下载文件'),
        url,
      }
    })
  ...
  const fallback = String(params.tencentOriginalResultUrl || task?.resultModelUrl || '').trim()
  if (!fallback) return []
  return [{ key: 'model', label: '主模型', url: fallback }]
}

function primaryDownloadUrl(task?: Model3dTask | null): string {
  return taskDownloads(task)[0]?.url || ''
}

function isTencentTask(task?: Model3dTask | null) {
  return String(task?.provider || '').includes('tencent')
}
  ...
  <div class="page">
    ...
    <div v-show="activeTab === 'create'" class="create-area">
      <aside class="form-panel">
        <!-- 任务模式：Tripo 提供商已绑定模式时隐藏 toggle，避免误导 -->
        <section class="fg" v-if="!isTripoProvider">
          <label class="fl">任务模式</label>
          <div class="mode-row">
            <button class="mode-btn" :class="{ active: form.taskType === 'text2model' }"
              @click="form.taskType = 'text2model'">文生 3D</button>
            <button class="mode-btn" :class="{ active: form.taskType === 'img2model' }"
              @click="form.taskType = 'img2model'">图生 3D</button>
          </div>
        </section>
        ...
        <section class="fg" v-if="form.taskType === 'img2model'">
          ...
          <!-- 已上传：紧凑缩略图 -->
          <div v-if="sourceImage" class="source-preview-compact">
          ...
          <ImageDropzone
            ...
            size="md"
            label="点击上传参考图"
            hint="单张 ≤10MB，支持 JPG / PNG / WebP"
            accept="image/*"
            :uploading="sourceUploading"
            @select="handleSourceFiles"
          />
        </section>
        ...
        <section class="fg">
          ...
          <a-textarea v-model="form.prompt" :auto-size="{ minRows: 3, maxRows: 7 }" :max-length="promptMaxLength"
            :placeholder="isTripoProvider && form.taskType === 'img2model'
              ? '可选：补充风格、材质或细节'
              : '例如：黑色狙击步枪，硬表面工业风，细节丰富'" />
        </section>
        ...
        <Model3dParamForm
          ...
          @update:value="updateProviderParams"
        />

        <div class="form-actions">
          <GenerateButton :loading="generating || uploading"
            :disabled="!(form.prompt.trim() || (isTripoProvider && form.taskType === 'img2model' && sourceImage))"
        ...
        <a-spin :loading="myLoading" class="works-spin" style="width:100%;min-height:200px">
          <div v-if="myTasks.length > 0" class="works-grid">
            <div v-for="task in myTasks" :key="task.id" class="card"
              ...
              <!-- 失败原因：截断+渐变，避免撑高卡片 -->
              <div v-if="task.status === 'failed' && task.errorMessage" class="task-error-row">
                <span class="task-error-text" :title="task.errorMessage">{{ task.errorMessage }}</span>
              </div>
    ...
    <a-modal v-model:visible="previewOpen" title="3D 详情预览" :width="980" :footer="false" unmount-on-close
      ...
      <div class="preview-wrap">
        <div class="viewer" ref="viewerRef">
          <div class="viewer-tools" v-if="previewTask && isTencentTask(previewTask)">
            <button class="vt-btn"
        ...
        <div class="detail" v-if="previewTask">
          ...
          <div class="detail-block">
            ...
            <div class="prompt-text">{{ previewTask.prompt || '无描述' }}</div>
          </div>

          <!-- 核心信息：精简为 4 项 -->
          <div class="detail-grid">
            <div class="item"><span class="k">状态</span><span class="v">{{ sText(previewTask.status) }}</span></div>
            <div class="item"><span class="k">模型</span><span class="v" :title="previewTask.provider || ''">{{ previewTask.provider || '-' }}</span></div>
          ...
          <!-- 结果文件下载 -->
          <div class="detail-block" v-if="taskDownloads(previewTask).length">
            <div class="kb">结果文件</div>
            <div class="download-list">
              <button
                v-for="item in taskDownloads(previewTask)"
                :key="item.key"
                class="download-item"
                @click="handleDownload(item.url)"
              >
                <span>{{ item.label }}</span>
                <span class="mono">下载</span>
              </button>
            </div>
          </div>
...
.source-preview-compact img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

/* 上传区已由 ImageDropzone 组件接管 */
