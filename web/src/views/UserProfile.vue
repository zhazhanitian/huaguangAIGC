    ...
    <div class="hero">
      ...
      <div class="hero-row">
        ...
        <a-popconfirm content="确定要退出登录吗？" type="warning" @ok="handleLogout">
          <a-button type="text" class="logout-btn"><IconPoweroff :size="14" /> 退出</a-button>
        </a-popconfirm>
      </div>
    </div>
      ...
      <div class="stats">
        <a-card class="stat-card" :bordered="true">
          <div class="stat-content">
            <div class="stat-icon gold"><IconDashboard :size="24" /></div>
        ...
        <a-card class="stat-card" :bordered="true">
          <div class="stat-content">
            <div class="stat-icon" :class="memberStatus.active ? 'purple' : 'gray'"><IconStar :size="24" /></div>
            <div class="stat-val">{{ memberStatus.label }}</div>
            <div class="stat-lbl">{{ memberStatus.desc }}</div>
            <a-button v-if="!memberStatus.active" class="upgrade-tag" type="text" size="mini" @click="openUpgrade">升级</a-button>
          </div>
        </a-card>
      </div>
      ...
      <a-tabs v-model:active-key="activeSection" class="main-tabs">
        <a-tab-pane key="profile" title="个人资料">
          <div class="pnl">
            <div class="pnl-top">
              <h3>个人资料</h3>
              <a-button v-if="!editing" type="outline" @click="editing=true"><IconEdit :size="15" /> 编辑</a-button>
            </div>
              ...
              <a-row :gutter="16">
                    ...
                    <div class="avatar-upload-row">
                      ...
                      >
                        <template #upload-button>
                          <a-button type="outline"><IconUpload /> 点击上传</a-button>
                        </template>
                      </a-upload>
                    </div>
                  </a-form-item>
                ...
                <a-col :span="24">
                  <a-form-item field="signature" label="个性签名">
                    <a-textarea v-model="form.signature" :disabled="!editing" :rows="3" placeholder="写下你的个性签名..." show-word-limit :max-length="100" />
                  </a-form-item>
                </a-col>
              </a-row>
            </a-form>
          ...
          <div class="pnl">
            ...
            <a-list :bordered="false" class="security-list">
              <a-list-item>
                <a-list-item-meta>
                  <template #avatar>
                    <div class="rw-icon"><IconLock :size="20" /></div>
                  </template>
        ...
        <a-tab-pane key="about" title="关于">
          <div class="pnl">
            <h3 class="ph">关于</h3>
            <p class="abt">华光 AIGC 创作平台 — 集 AI 对话、绘画、视频、音乐于一体的智能创作工具。</p>
            <div class="abt-l">
...
.hero-row { position: relative; z-index: 1; display: flex; align-items: center; gap: 18px; }
.hero-parallax {
  position: absolute;
  inset: 0;
  z-index: 0;
...
.av-c { 
  ...
  font-size: 24px; font-weight: 700; color: var(--profile-accent-primary);
}
...
.since { color: var(--profile-text-soft); }
.logout-btn { color: var(--profile-text-secondary); }
.logout-btn:hover { color: var(--color-danger-6); }
...
/* === 统计卡片 2 列 或 4 列 === */
.stats { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 0; }
.stat-card {
...
.stat-content { position: relative; padding: 18px; display: flex; flex-direction: column; gap: 4px; }
.stat-icon { width: 44px; height: 44px; border-radius: 10px; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; }
.gold { background: color-mix(in srgb, var(--profile-accent-warning) 18%, var(--color-bg-2)); color: var(--profile-accent-warning); }
.purple { background: color-mix(in srgb, var(--profile-accent-primary) 18%, var(--color-bg-2)); color: var(--profile-accent-primary); }
.gray { background: var(--color-fill-3); color: var(--color-text-2); }
.teal { background: color-mix(in srgb, var(--profile-accent-success) 18%, var(--color-bg-2)); color: var(--profile-accent-success); }
.pink { background: color-mix(in srgb, var(--color-danger-6) 18%, var(--color-bg-2)); color: var(--color-danger-6); }
.stat-val {
...
.stat-lbl { font-size: 0.8rem; color: var(--profile-text-soft); line-height: 1.45; }
.upgrade-tag {
  position: absolute;
  top: 10px;
  right: 10px;
...
.upgrade-tag:hover { background: color-mix(in srgb, var(--color-primary-6) 16%, transparent); }

/* === TAB === */
.main-tabs :deep(.arco-tabs-nav) { border-bottom: 1px solid var(--color-border); margin-bottom: 0; }
.main-tabs :deep(.arco-tabs-content) { padding-top: 16px; }
.main-tabs :deep(.arco-tabs-tab) { font-size: 0.92rem; padding: 12px 20px; }
.main-tabs :deep(.arco-tabs-tab-active) { color: var(--profile-accent-primary); font-weight: 600; }
.main-tabs :deep(.arco-tabs-tab-active .arco-tabs-tab-title::after) { content: ''; position: absolute; bottom: -1px; left: 20px; right: 20px; height: 2px; background: var(--profile-accent-primary); border-radius: 1px; }
...
.pnl-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; gap: 10px; }
.pnl-top h3, .ph {
  margin: 0;
  font-size: 1.02rem;
  line-height: 1.45;
...
.avatar-fallback { background: color-mix(in srgb, var(--profile-accent-primary) 16%, var(--color-bg-2)); color: var(--profile-accent-primary); }
.modal-acts { display: flex; justify-content: flex-end; gap: 12px; margin-top: 24px; }
...
.security-list :deep(.arco-list-item:not(:last-child)) { margin-bottom: 8px; }
.rw-icon {
  width: 38px;
  height: 38px;
  border-radius: 10px;
...
.security-list :deep(.arco-list-item-meta-title) { font-size: 0.9rem; font-weight: 500; color: var(--color-text-1); }
.security-list :deep(.arco-list-item-meta-description) { font-size: 0.78rem; color: var(--profile-text-soft); margin-top: 2px; }

/* 关于 */
.abt { font-size: 0.95rem; color: var(--profile-text-muted); line-height: 1.72; margin: 0 0 12px; max-width: 64ch; }
.abt-l { display: flex; align-items: center; gap: 10px 12px; flex-wrap: wrap; font-size: 0.82rem; color: var(--profile-text-soft); }
.abt-l .link { color: var(--profile-accent-primary); text-decoration: none; font-weight: 500; }
.abt-l .link:hover { text-decoration: underline; }
...
@media(max-width: 600px) { 
  ...
  .hero-info h1 { font-size: 1.1rem; } 
  .body, .hero { padding-left: 16px; padding-right: 16px; } 
  .pnl-top { flex-wrap: wrap; }
  .acts,
  .modal-acts { width: 100%; }
  .acts :deep(.arco-btn),
  .modal-acts :deep(.arco-btn) { flex: 1; }
}
