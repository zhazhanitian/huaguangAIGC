...
const imageModelsFromApi = ref<Array<{ modelName: string; displayName?: string | null; description?: string }>>([])
const videoModelsFromApi = ref<Array<{ modelName: string; displayName?: string | null; description?: string }>>([])
...
const visibleImageProviders = computed(() => {
  ...
  for (const p of imageProvidersDef) {
    const backends = imageProviderToBackendNames[p.value]
    const names = Array.isArray(backends) ? backends : [backends ?? p.value]
  ...
  const baseList = set.size === 0
    ? [...imageProvidersDef, ...dynamicProviders]
...
const visibleVideoProviders = computed(() => {
  ...
  const knownNames = new Set<string>()
  for (const p of videoProvidersDef) {
    const backends = videoProviderToBackendNames[p.value]
    const names = Array.isArray(backends) ? backends : [backends ?? p.value]
    names.forEach((name) => knownNames.add(name))
  }
  ...
  const baseList = set.size === 0
    ? [...videoProvidersDef, ...dynamicProviders]
...
async function fetchModelPoints() {
  try {
    ...
    if (Array.isArray(imageList)) {
      imageModelsFromApi.value = imageList
    ...
    if (Array.isArray(videoList)) {
      videoModelsFromApi.value = videoList
