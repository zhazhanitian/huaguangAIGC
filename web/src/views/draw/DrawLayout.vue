...
import MapiImageParamForm from './MapiImageParamForm.vue'
import { resolveMapiImageSchema } from './mapi-image-schemas'
import { estimateImagePrice, extractPriceList } from './mapi-pricing-client'
...
const imageModelsFromApi = ref<Array<{
  modelName: string
...
/** 当前所选 MAPI 模型的 rawMetadata（用于前端实时价格预估） */
const currentMapiRawMetadata = computed<string | null>(() => {
  const hit = imageModelsFromApi.value.find((m) => m.modelName === actualProvider.value)
...
/** 来自后端 source=mapi 的模型名集合，用于判断当前是否走 MAPI 聚合协议 */
const mapiImageModelSet = computed(() => {
  const s = new Set<string>()
  for (const m of imageModelsFromApi.value) {
...
/** MAPI 图片：按 schema 驱动的扁平表单值；切换模型时由 MapiImageParamForm 内部重置 */
const mapiForm = ref<Record<string, unknown>>({})
/** 当前 schema（基于 actualProvider 解析） */
const mapiSchema = computed(() => resolveMapiImageSchema(actualProvider.value))
...
/** 当前所选模型是否走 MAPI 聚合协议：
 *  - 后端声明 source=mapi
 *  - 且本地 schemas 里有对应定义（没 schema 的 MAPI 模型视作不支持，不走 MAPI 分支）
 */
const isMapiProvider = computed(
  () =>
    mapiImageModelSet.value.has(actualProvider.value) &&
    !!resolveMapiImageSchema(actualProvider.value),
)

/** MAPI 图片实时价格预估（在提交按钮旁展示） */
const mapiPriceEstimate = computed(() => {
  if (!isMapiProvider.value) return null
  const list = extractPriceList(currentMapiRawMetadata.value)
  if (!list.length) return null
  const n = Math.max(1, Number((mapiForm.value as Record<string, unknown>).n ?? 1))
  return estimateImagePrice(list, n)
})
...
const visibleProviders = computed(() => {
  ...
  const dynamicProviders = imageModelsFromApi.value
  ...
  // 过滤掉「source=mapi 但本地没有 schema」的模型，避免出现空白参数面板
  const mapiSet = mapiImageModelSet.value
  const filteredBase = baseList.filter((p) => {
    const backends = imageProviderToBackendNames[p.value]
    const names = Array.isArray(backends) ? backends : [backends ?? p.value]
    for (const name of names) {
      if (mapiSet.has(name) && !resolveMapiImageSchema(name)) return false
    }
    return true
  })
...
const modelTagInfo: Record<string, { color: string; label: string }> = {
  'nano-banana-pro': { color: 'orangered', label: 'Nano Banana Pro' },
  'gpt-image-1': { color: 'green', label: 'GPT Image 1' },
  'gpt-image-1.5': { color: 'green', label: 'GPT Image 1' },
  'doubao-seedance-4-5': { color: 'arcoblue', label: 'Doubao Seedance 4.5' },
  'flux-2-pro': { color: 'cyan', label: 'Flux Pro' },
  'flux-kontext-pro': { color: 'cyan', label: 'Flux Kontext' },
  'flux-kontext-max': { color: 'cyan', label: 'Flux Kontext Max' },
  'z-image': { color: 'lime', label: 'Z-Image' },
  'grok-imagine/text-to-image': { color: 'magenta', label: 'Grok Imagine' },
  'qwen/text-to-image': { color: 'purple', label: '通义万相·文生图' },
  'qwen/image-to-image': { color: 'purple', label: '通义万相·图转图' },
  'qwen/image-edit': { color: 'purple', label: '通义万相·编辑' },
...
const providerDisplayName = computed(() => {
  const p = actualProvider.value
  const tag = modelTagInfo[p]
  if (tag) return tag.label
  // 动态 MAPI 模型：读 displayName；兜底直接显示 modelName
  const apiModel = imageModelsFromApi.value.find((m) => m.modelName === p)
  return apiModel?.displayName || p
})
...
async function handleGenerate(forceGenerate = false) {
  ...
  try {
    ...
    // === MAPI 聚合短路：按 schema 驱动，直接把 mapiForm 当 params 上游透传 ===
    if (isMapiProvider.value && mapiSchema.value) {
      const form = JSON.parse(JSON.stringify(mapiForm.value || {})) as Record<string, unknown>

      // 参考图 -> 按 schema 指定的 key 注入（Seedream 用 image、Nano Banana 用 urls）
      const refUrls = refImages.value.map((r) => normalizeRefUrl(r.url)).filter(Boolean)
      const refField = mapiSchema.value.fields.find((f) => f.type === 'ref-images')
      if (refField && refUrls.length > 0) {
        if (refField.key === 'urls') {
          form.urls = refUrls
        } else if (refField.key === 'image') {
          // Seedream 支持单图 string 或多图 array
          form.image = refUrls.length === 1 ? refUrls[0] : refUrls
        } else {
          form[refField.key] = refUrls
        }
      }

      // 丢弃渲染器内部草稿字段
      delete form.__customSize__

      payload.params = form
      const { data: mapiData } = await createDrawTask(payload)
      if (mapiData) {
  ...
  <div class="page">
    ...
    <div v-show="activeTab === 'create'" class="create-area">
      ...
      <aside class="form-panel">
        ...
        <!-- 子模型选择（有多个子模型时显示）-->
        <section v-if="currentConfig.subModels && currentConfig.subModels.length > 1 && !isMapiProvider" class="form-group">
        ...
        <!-- 参考图（图生图）——非 MAPI 模型使用；MAPI 由 MapiImageParamForm 内部接管 -->
        <section v-if="maxRefImages > 0 && !isMapiProvider" class="form-group">
          ...
          <ImageDropzone
            ...
            size="lg"
            label="点击或拖拽上传参考图"
            :hint="`支持 JPG / PNG / WebP，单张 ≤10MB，最多 ${maxRefImages} 张`"
            accept="image/*"
            multiple
        ...
        <!-- 反向提示词（可折叠）——MAPI 图片文档里不支持 negative prompt，故对 MAPI 隐藏 -->
        <template v-if="!isMapiProvider">
        ...
        <!-- MAPI 聚合：按 schema 逐模型渲染全部字段 -->
        <section v-if="isMapiProvider" class="form-group mapi-form-group">
          <MapiImageParamForm
            :model-name="actualProvider"
            :raw-metadata="currentMapiRawMetadata"
            v-model:value="mapiForm"
            v-model:ref-images="refImages"
          />
        </section>

        <!-- 比例/尺寸选择（部分模型隐藏；MAPI 模型由上方 MAPI 面板接管，不再显示此块）-->
        <section v-if="showRatioSelect && !isMapiProvider" class="form-group">
        ...
        <section v-if="currentConfig.sizes && currentConfig.sizes.length > 1 && !isMapiProvider" class="form-group">
        ...
        <section v-if="currentConfig.resolutions && currentConfig.resolutions.length > 0 && !isMapiProvider" class="form-group">
        ...
        <section v-if="currentConfig.responseFormats && currentConfig.responseFormats.length > 0 && !isMapiProvider" class="form-group">
        ...
        <section v-if="currentConfig.variants && !isMapiProvider" class="form-group">
        ...
        <div class="form-actions">
          <div v-if="isMapiProvider && mapiPriceEstimate" class="mapi-price-hint">
            预估消耗 <strong>{{ mapiPriceEstimate.points }}</strong> 积分
          </div>
...
.work-card {
  ...
  overflow: hidden;
  cursor: pointer;
  transition: box-shadow var(--duration-normal) var(--ease-out), border-color var(--duration-normal), transform var(--duration-normal) var(--ease-out);
  position: relative;
  display: flex;
  flex-direction: column;
