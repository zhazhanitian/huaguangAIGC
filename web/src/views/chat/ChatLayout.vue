...
/** 对话页展示的模型：来自后端 GET /model/list?type=text，仅展示 isActive 的，不再前端白名单过滤 */
...
async function fetchChatModels() {
  try {
    ...
    if (Array.isArray(all) && all.length > 0) {
      const chatModels = all.filter(m => m.isActive)
      if (chatModels.length > 0) {
        models.value = chatModels
        selectedModel.value = chatModels[0]?.modelName
      }
    }
  } catch { /* use defaults */ }
}
    ...
    <header class="topbar">
      <div class="topbar-left">
        ...
        <div class="conv-picker" @click="showConvPanel = !showConvPanel">
          ...
          <svg width="12" height="12" viewBox="0 0 12 12" class="conv-arrow" :class="{ open: showConvPanel }">
            <path d="M3 4L6 7L9 4" stroke="currentColor" stroke-width="1" fill="none" stroke-linecap="round" />
          </svg>
        </div>
        <a-button class="topbar-btn" type="text" @click="handleNewChat" title="新建对话">
          <IconPlus :size="16" />
        </a-button>
      </div>
      <div class="topbar-right">
        <div class="model-sel-wrap">
          <a-select v-model="selectedModel" size="small" :style="{ width: '100%' }">
            <a-option v-for="m in models" :key="m.id" :value="m.modelName" :label="m.displayName || m.modelName">
              <div class="ui-option">
                <div class="ui-option-main">
                  <div class="ui-option-header">
                    <span class="ui-option-title">{{ m.displayName || m.modelName }}</span>
        ...
        <div class="conv-panel-inner">
          ...
          <div class="conv-scroll">
            <template v-for="group in groupedConversations" :key="group.label">
              ...
              <div v-for="item in group.items" :key="item.id" class="conv-row"
                :class="{ active: chatStore.currentGroup?.id === item.id }"
                @click="selectGroup(item); showConvPanel = false" @mouseenter="hoverGroup = item.id"
                @mouseleave="hoverGroup = null">
                <span class="conv-name">{{ item.title || '新对话' }}</span>
                <button v-show="hoverGroup === item.id" class="conv-x" @click.stop="renameGroup($event, item)"
                  title="重命名">
                  <IconEdit :size="13" />
                </button>
                <button v-show="hoverGroup === item.id" class="conv-x" @click.stop="deleteGroup($event, item.id)">
                  <IconDelete :size="13" />
                </button>
              </div>
            </template>
      ...
      <div ref="msgListRef" class="msg-scroll" @scroll="handleMsgScroll">
        ...
        <div v-if="isWelcome" class="welcome">
          <div class="welcome-logo">
            <img :src="huaguangLogo" alt="网龙 logo" />
          </div>
        ...
        <!-- 消息列表 -->
        <template v-else>
          <div v-for="(msg, mi) in chatStore.messages" :key="msg.id" class="msg-row"
            :class="[msg.role, { hover: hoverMsg === msg.id }]" @mouseenter="hoverMsg = msg.id"
            ...
            <div class="msg-inner">
              ...
              <div class="msg-body">
                ...
                <div class="msg-content">
                  <template v-if="msg.role === 'user'">
                    ...
                    <div v-if="msg.attachments?.length" class="msg-attachments">
                      <button v-for="a in msg.attachments" :key="a.id" class="msg-attachment" type="button"
                  ...
                  <template v-else>
                    <div v-if="isStreamingAssistantMsg(msg, mi)" class="stream-feedback"
                    ...
                    <template v-if="true" v-for="(_, __) in [parsedContentByMessage(msg, mi)]" :key="0">
                      <!-- 深度思考折叠块 -->
                      <div v-if="_.think" class="think-box"
                        :class="{ 'is-streaming': isStreamingAssistantMsg(msg, mi) }">
                        <button class="think-btn" :class="{ 'is-live': isStreamingAssistantMsg(msg, mi) }"
                          @click="thinkExpanded[msg.id] = !thinkExpanded[msg.id]">
                          ...
                          <svg width="12" height="12" viewBox="0 0 12 12" :class="{ flip: thinkExpanded[msg.id] }">
                            <path d="M3 4L6 7L9 4" stroke="currentColor" stroke-width="1" fill="none"
                              stroke-linecap="round" />
                          </svg>
                        </button>
                        <div v-show="thinkExpanded[msg.id]" class="think-body"
                          :class="{ 'is-live': isStreamingAssistantMsg(msg, mi) }">
                ...
                <div v-show="hoverMsg === msg.id && !chatStore.streaming" class="msg-actions">
                  <button class="act-btn" :class="{ copied: copiedId === msg.id }"
                    @click.stop="copyMsg(msg.content, msg.id)">
                    <IconCopy :size="14" />
                  ...
                  <template v-if="msg.role === 'assistant'">
                    ...
                    <button class="act-btn react-btn" :class="{ activeDown: msgReactions[msg.id] === 'down' }"
                      :disabled="reactionSending[msg.id]" @click.stop="toggleReaction(msg, 'down')">
      ...
      <div class="input-wrap" @dragover.prevent @drop="handleFileDrop">
        <div class="input-box">
          ...
          <div class="input-row">
            ...
            <a-textarea ref="inputTextareaRef" v-model="inputText" class="input-textarea" placeholder="给 网龙AI 发送消息..."
          ...
          <input ref="fileInputRef" type="file" multiple
            accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml,application/pdf,text/plain,application/json"
...
.conv-search :deep(.arco-input::placeholder) {
  color: var(--text-4);
}
...
.panel-slide-leave-active {
  animation: panelIn 0.14s ease-in reverse;
}
...
.panel-slide-enter-active .conv-panel-inner {
  animation: panelSlide 0.22s var(--ease-out);
}

.panel-slide-leave-active .conv-panel-inner {
  animation: panelSlide 0.16s ease-in reverse;
}
...
.stream-bars i:nth-child(2) {
  animation-delay: 0.12s;
}

.stream-bars i:nth-child(3) {
  animation-delay: 0.4s;
}

.stream-feedback.phase-waiting .stream-bars {
  color: #38bdf8;
}

.stream-feedback.phase-thinking .stream-bars {
  color: #B2D4FF;
}

.stream-feedback.phase-answering .stream-bars {
  color: #00B42A;
}
...
.msg-actions {
  ...
  gap: 2px;
  margin-top: var(--sp-2);
  animation: fadeUp var(--duration-fast) var(--ease-out);
}
...
.input-textarea :deep(textarea:disabled) {
  opacity: 0.7;
}
