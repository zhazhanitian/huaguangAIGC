...
export class VideoController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: '管理员-获取所有视频任务',
    description: '支持 ?status=processing&staleMinutes=30 过滤孤儿任务',
  })
  async adminGetAllTasks(
    ...
    const page = query.page ?? 1;
    const pageSize = query.pageSize ?? 20;
    const staleMin = staleMinutes ? parseInt(staleMinutes, 10) : undefined;
    return this.videoService.getAllTasks(page, pageSize, status, staleMin);
  }

  @Post('admin/tasks/:id/force-fail')
  @UseGuards(JwtAuthGuard, AdminGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '管理员-强制终止单个视频任务（标为 failed）' })
  async adminForceFailTask(
    @Param('id') taskId: string,
    @Body('reason') reason?: string,
  ) {
    return this.videoService.adminForceFailTask(taskId, reason);
  }
}
