...
export class OpsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    // 启动时检测队列是否被禁用，若禁用则告警
    const queueEnabled =
      String(process.env.TASK_QUEUE_ENABLED ?? 'true').trim().toLowerCase() !== 'false';
    if (!queueEnabled) {
      this.logger.warn(
        '⚠️  TASK_QUEUE_ENABLED=false：任务队列已禁用，服务重启后进行中的任务将丢失！' +
          '建议生产环境改为 true（需要 Redis）。',
      );
    }
    ...
    this.logTicker = setInterval(() => {
      this.getStats({})
        .then((s) => {
          const q = s.queues;
          const queueTag = queueEnabled ? '' : ' [QUEUE_DISABLED]';
          this.logger.log(
            `ops${queueTag}: wsClients=${s.ws.connectedClients} rooms=${s.ws.rooms} ` +
