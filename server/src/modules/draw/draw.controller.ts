...
export class DrawController {
  ...
  async adminGetAllTasks(
    ...
    return this.drawService.getAllTasks(page, pageSize, status, staleMin);
  }

  @Post('admin/tasks/:id/force-fail')
  @UseGuards(JwtAuthGuard, AdminGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '管理员-强制终止单个任务（标为 failed）' })
  async adminForceFailTask(
    @Param('id') taskId: string,
    @Body('reason') reason?: string,
  ) {
    return this.drawService.adminForceFailTask(taskId, reason);
  }
}
