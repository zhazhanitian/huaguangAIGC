...
import {
  createMapiGrsaiNanoBananaTask,
  createMapiImageTask,
  waitMapiImage,
  type MapiImagesRequest,
...
export class DrawService {
  ...
  /**
   * 根据 MAPI 模型的 rawMetadata 价目和请求 params，估算预扣积分。
   * - MAPI 源：按 appPriceModelList 精确算
   * - 非 MAPI 源：回退 ai_models.deductPoints
   * - 图片 FIX/FIX：预扣 = 实际 = n × unitPrice
   */
  private async resolvePointsForMapi(
    ...
    if (list.length === 0) {
      return {
        points: m.deductPoints > 0 ? m.deductPoints : POINTS_PER_DRAW_FALLBACK,
        mapi: true,
        breakdown: 'MAPI 但无价目，回退 deductPoints',
      };
    }
    const n = Math.max(1, Number(params?.n ?? 1));
    const est = estimateMaxPrice(list, 'image', { n });
    return {
  ...
  async createTask(userId: string, dto: CreateDrawTaskDto): Promise<DrawTask> {
    ...
    // MAPI 模型按价目表估算预扣；非 MAPI 回退 deductPoints
    ...
    this.logger.log(
      `[Draw 预扣] model=${providerName} points=${deductPoints} ${pricing.mapi ? '[MAPI]' : ''} ${pricing.breakdown}`,
    );
  ...
  async processDrawTask(task: DrawTask): Promise<void> {
    try {
      ...
      // MAPI 聚合：若 ai_models 里该模型 source=mapi，优先走 MAPI 统一图片接口
      const isMapiModel = await this.isMapiSourceModel(task.provider);
      const channel = resolveImageProviderChannel(task.provider, isMapiModel);
      if (channel === 'mapi') {
        imageUrl = await this.processMapiImage(task);
      } else {
  ...
  /**
   * MAPI 聚合平台图片生成
   * 文档：POST /Mapi/v3/images/generations + GET /Mapi/v3/images/generations?ourOrderNo=xxx
   * 前端应传字段：size / watermark / inputImage（图生图时）
   */
  ...
  /**
   * 绘画 / 画布等内部字段，不应出现在 MAPI 上游请求体里。
   * 注意：aspectRatio 不在此列表中——Nano Banana 等模型需要透传；
   * 对 Seedream 系列则由下方循环额外过滤。
   */
  private readonly MAPI_IMAGE_RESERVED_KEYS = new Set<string>([
    'style',
    '__processingAtMs',
    '__endedAtMs',
    '__taskSource',
    // 前端旧字段：由我们转换成 schema 规范字段（image / urls）
    'inputImage',
    'sourceImageUrl',
    'imageUrl',
    'imageUrls',
  ...
  private async processMapiImage(task: DrawTask): Promise<string> {
    const params = (task.params || {}) as Record<string, unknown>;
    const prompt = String(task.prompt || '').trim();
    if (!prompt) {
      throw new BadRequestException('MAPI 图片生成缺少提示词 prompt');
    }

    // 发给 MAPI 的 model 参数 = 权威 rawMetadata.modelName（大小写敏感，如 "Kling-3.0"）
    // 回退顺序：rawMetadata.modelName -> ai_models.modelName -> task.provider
    const matchedModels = await this.modelRepository.find({
      where: { modelName: task.provider },
      order: { updatedAt: 'DESC', createdAt: 'DESC' },
    });
    const aiModel = matchedModels[0] ?? null;
    const upstreamModel = resolveConsistentUpstreamModelName(
      matchedModels,
      task.provider,
    );

    // 识别 Nano Banana 系列：字段用 aspectRatio / imageSize / urls
    const isNanoBanana = isNanoBananaModel(task.provider);

    // 1) 透传 params 里所有有效字段（跳过内部保留 key）
    const passthrough: Record<string, unknown> = {};
    for (const [k, v] of Object.entries(params)) {
      if (this.MAPI_IMAGE_RESERVED_KEYS.has(k)) continue;
      if (v === undefined || v === null || v === '') continue;
      // Nano Banana 保留 aspectRatio，其他模型不传（避免上游报错）
      if (k === 'aspectRatio' && !isNanoBanana) continue;
      passthrough[k] = v;
    }

    // 2) 兼容旧字段：前端若没在 schema 里填图片，把 inputImage/sourceImageUrl/imageUrls[0] 兑现为对应字段
    const legacyImage =
      (params.inputImage as string) ||
      (params.sourceImageUrl as string) ||
      (Array.isArray(params.imageUrls)
        ? String((params.imageUrls as string[])[0] || '')
        : '') ||
      (params.imageUrl as string) ||
      '';
    if (legacyImage) {
      if (isNanoBanana && !passthrough.urls) {
        passthrough.urls = [legacyImage];
      } else if (!isNanoBanana && !passthrough.image) {
        passthrough.image = legacyImage;
      }
    }

    const req: MapiImagesRequest = {
      model: upstreamModel,
      prompt,
      ...passthrough,
      // 隐藏的技术字段：前端不再暴露，后端统一设默认值
      response_format: 'url',
      stream: false,
    };

    this.logger.log(
      `[MAPI 图片] 创建任务: model=${req.model}, taskId=${task.id}, ` +
        `fields=[${Object.keys(passthrough).join(',') || '-'}], ` +
        `sourceModel=${aiModel?.id || '-'}:${aiModel?.source || '-'}:${extractUpstreamModelName(aiModel?.rawMetadata) || aiModel?.modelName || '-'}`,
    );
    ...
    if (isNanoBanana) {
      ...
      await this.reconcileMapiImageBilling(
        task,
        aiModel,
        result as unknown as { usage?: { generated_images?: number } },
        Number(passthrough.n ?? 1),
      );
      this.logger.log(`[MAPI 图片] 完成(Grsai): taskId=${task.id}, url=${url}`);
      return url;
    }
    ...
    const { url, result } = await waitMapiImage(ourOrderNo, {
      intervalMs: 3000,
  ...
  /** 图片任务完成后：按 MAPI usage.generated_images 重算实际积分，多退少补 */
  private async reconcileMapiImageBilling(
    task: DrawTask,
    aiModel: AiModel | null,
    result: { usage?: { generated_images?: number } } | null,
    requestedN: number,
  ): Promise<void> {
    try {
      ...
      const actualN = Math.max(
        1,
        Number(result?.usage?.generated_images ?? requestedN ?? 1),
      );
      const actual = computeCallPrice(list, { kind: 'image', n: actualN });
  ...
  async deleteTask(userId: string, taskId: string): Promise<void> {
    ...
    if (task.status === DrawTaskStatus.PENDING) {
  ...
  /**
   * 管理员：获取所有任务（分页）。
   * @param status 可选，按状态过滤（pending / processing / completed / failed）
   * @param staleMinutes 可选，只返回 updatedAt 超过此分钟数仍处于 pending/processing 的孤儿任务
   */
  async getAllTasks(
    page: number = 1,
    pageSize: number = 20,
    ...
    if (staleMinutes && staleMinutes > 0) {
      ...
      qb.andWhere('t.status IN (:...staleStatuses)', {
        staleStatuses: [DrawTaskStatus.PENDING, DrawTaskStatus.PROCESSING],
  ...
  async adminForceFailTask(taskId: string, reason?: string): Promise<DrawTask> {
    ...
    this.logger.warn(
      `[adminForceFailTask] 强制终止任务 id=${taskId} userId=${task.userId} provider=${task.provider} reason="${errorMsg}"`,
    );

    return updated as DrawTask;
  }
}
