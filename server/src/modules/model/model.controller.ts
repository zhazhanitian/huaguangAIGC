...
export class ModelController {
  ...
  @Post('admin/sync-mapi')
  @UseGuards(AdminGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '从 MAPI 同步模型目录到数据库（管理端）' })
  async syncMapi() {
    return this.modelService.syncMapiModels();
  }

  @Post('admin/bulk-disable')
  @UseGuards(AdminGuard)
  @ApiBearerAuth()
  @ApiOperation({
    summary: '批量禁用模型（管理端）',
    description:
      '支持两种模式：①传 { ids: string[] } 精确禁用；②传 { sourceNot: "mapi" } 将所有 source != mapi 的模型 isActive/isPublic 置为 false',
  })
  async bulkDisable(
    @Body() dto: { ids?: string[]; sourceNot?: string },
  ) {
    return this.modelService.bulkDisableModels(dto);
  }
