...
export class AiModel {
  ...
  @Column({
    length: 120,
    nullable: true,
    comment: '展示名称，用于前后端显示；为空时回退 modelName',
  })
  ...
  @Column({ type: 'tinyint', default: 1, comment: '是否在前台模型广场显示' })
  isPublic: boolean;

  @Column({
    length: 32,
    default: 'local',
    comment: '模型来源，如 local / preset / mapi',
  })
  source: string;
  ...
  @Column({
    type: 'text',
    nullable: true,
    comment: '模型能力标签 JSON，例如 [\"image-edit\",\"text-to-video\"]',
  })
